...
def test_configure_container_works_with_remote_mount_source() -> None:
    ...
    assert "INBOX_SOURCE_TYPE" not in command_text
...
def test_post_start_uses_plain_upload_probe_for_remote_mount_metadata() -> None:
    plugin = service_class()(host_port=unused_port())
    ...
    assert "/api/inbox/upload" in container.commands[0][2]
