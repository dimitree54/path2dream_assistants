...
- When `_state` is `"mounted"`, `/status` must perform live mount health verification before returning JSON. This verification must check that the container path is still a mountpoint, that the mounted filesystem is readable, and that the configured rclone remote is accessible. If live verification fails, `/status` must transition to `error` state and report `authValid=false`, `mounted=false`, `state="error"` with a descriptive message. Live `/status` verification must not perform write-through probes to avoid unnecessary remote writes on frequent status checks.
